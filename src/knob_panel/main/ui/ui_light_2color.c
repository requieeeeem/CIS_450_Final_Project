...
static bool light_2color_layer_enter_cb(void *layer)
{
    ...
    if (NULL == create_layer->lv_obj_layer) {
        ...
        audio_event_group = xEventGroupCreate(); //audio event task creation
        if (audio_event_group == NULL) {
            printf("Error: Failed to create event group");
        }

        xTaskCreate( //create new task
            voice_announcement_task, //voice announcement function
            "VoiceAnnouncement", //string name for easy debug
            4096, //memory allocated
            NULL, //no parameter passed
            5, //priority, 5 is pretty high
...
static void light_2color_layer_timer_cb(lv_timer_t *tmr)
{
    ...
    if (is_time_out(&time_20ms)) {
        ...
        if ((light_set_conf.light_pwm ^ light_xor.light_pwm) || (light_set_conf.light_cck ^ light_xor.light_cck)) {
            ...
            switch (light_xor.light_pwm) { //check light status, then set bit to trigger voice_announcement_task
            case 100:
...
static void voice_announcement_task(void* arg) { //wait for the bits to be triggered, then play audio depends on what was triggered
    while (1) {
        EventBits_t trigger = xEventGroupWaitBits(
            ...
            pdTRUE, //clear bits after function was called
            pdFALSE, //doesn't wait for all bits
